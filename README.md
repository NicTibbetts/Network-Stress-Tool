...
If you run a web service and want to know how it behaves under pressure before your users find out for you, load testing against your own infrastructure is exactly what it was built for. Finding out that your server falls over at 400 concurrent connections during a controlled test on a random afternoon is a much better outcome than finding out during a product launch. Used that way (against systems you own or have written authorization to test, with a clear scope) this tool gives you signal that synthetic benchmarks don't: real HTTP stack behavior, real TLS negotiation overhead, real connection exhaustion patterns.
...
| `-cache-bust` | true | appends unique query parameters (`_t=`, `rand=`, etc.) to every URL so CDN and proxy caches cannot serve a cached response. forces the origin server to handle each request. |
...
| `-mimic-behavior` | false | on VOLUME_ATTACK (type 0), replaces the raw request loop with a simulated user session: think-time pauses between requests, realistic session durations (5 minutes), variable page view counts, and 40% bounce rate simulation. makes traffic patterns look like a real user rather than a flood. |
...
### UDP reflection / amplifier discovery

| flag | default | what it does |
|---|---|---|
| `-dns-amplifiers <file>` | none | Load an explicit DNS reflector inventory from a text file of one IP per line. |
| `-ntp-amplifiers <file>` | none | Load an explicit NTP monlist reflector inventory from a text file. |
| `-memcached-amplifiers <file>` | none | Load an explicit memcached reflector inventory from a text file. |
| `-ssdp-amplifiers <file>` | none | Load an explicit SSDP reflector inventory from a text file. |
| `-discover-amplifiers` | false | Probe random public IPs for live DNS/NTP/memcached/SSDP amplifiers before the attack starts. |
| `-discover-timeout <sec>` | 45 | Budget for auto-discovery. |
| `-discover-count <n>` | 50 | How many live amplifiers to keep per protocol. |
| `-discover-save` | false | Save the discovered sets to `amplifiers_<protocol>.txt` for reuse across runs. |
| `-use-built-in-reflectors` | false | Load the bundled reflector seed corpus from [assets/bundled_reflector_corpus.txt](assets/bundled_reflector_corpus.txt) for type 10. Leave this off to force explicit inventories only. |

A seed list is just a plain text inventory of candidate reflector IPs, one per line. The file format is intentionally simple: blank lines and `#` comments are ignored, and Shodan/Censys-style exports work directly. This is the file you supply yourself when you already have a verified reflector inventory to feed the reflection path.

The two inventory paths are easy to confuse:

1. `-dns-amplifiers`, `-ntp-amplifiers`, `-memcached-amplifiers`, `-ssdp-amplifiers`, `-chargen-amplifiers`, and `-qotd-amplifiers` all point at your own text file(s). These are the explicit inventories you curate yourself.
2. `-use-built-in-reflectors` loads the repo's bundled starter corpus from [assets/bundled_reflector_corpus.txt](assets/bundled_reflector_corpus.txt). That file is a seed inventory shipped with the project, not a live reflector broker.

A starter template for your own inventory lives in [assets/amplifier_seed_example.txt](assets/amplifier_seed_example.txt). You can point the reflection mode at it with any of the `-*-amplifiers` flags, for example:

The runtime now also has a bundled seed corpus at [assets/bundled_reflector_corpus.txt](assets/bundled_reflector_corpus.txt), and `-use-built-in-reflectors` fills missing reflector vectors from that corpus before discovery runs. So the real supply paths are:

1. explicit file lists with `-dns-amplifiers`, `-ntp-amplifiers`, and the other `-*-amplifiers` flags;
2. live discovery with `-discover-amplifiers` / `-discover-save`; and
3. the bundled corpus via `-use-built-in-reflectors`.

These three paths work together: explicit files still win, discovery still extends what you already have, and the bundled corpus gives the reflection mode a real starting inventory instead of an empty one.

```sh
./demon -udp-reflection \
  -dns-amplifiers assets/amplifier_seed_example.txt \
  -discover-amplifiers \
  -discover-save \
  <target>
```

The `-discover-amplifiers` path probes public IPv4 space for live responders and writes reusable `amplifiers_<protocol>.txt` files when `-discover-save` is set. It is best-effort only: it can find live responders on a good network path, but on a constrained host or heavily filtered internet path it will often return zero. In practice, the reliable path is to supply your own verified inventory file first, then use discovery only as a supplement.
| `-udp-reflection` | false | **Opt into** the reflection/amplification attack mode (raw sockets, spoofed source IP, third-party amplifiers). Needs root / `CAP_NET_RAW`. Off by default — the default type-10 mode is the direct, non-spoofed flood against your target. |
| `-udp-direct` | false | Force the direct (non-spoofed) flood against the target only. This is already the default mode; the flag pins it and overrides `-udp-reflection`. |
| `-udp-pps <n>` | 0 (=100k) | Direct flood: aggregate packet-rate cap in packets/sec across all senders. Raise it to find the target's real pps ceiling instead of the tool's; it's logged and flagged in the dashboard when binding. |

> `-bandwidth` and `-adaptive-bandwidth` pace UDP egress and are documented under [Bandwidth / congestion control](#bandwidth--congestion-control) below.

### UDP measurement (own-endpoint load testing)

UDP has no acknowledgement, so a successful send only means the datagram left your box — not that the target received it. Run the binary as a receiver agent **on a host you control** and pair it with `-receiver-stats` on the sender to measure what actually lands: real pps/bps absorbed and packet loss.

| flag | default | what it does |
|---|---|---|
| `-udp-receiver <addr>` | none | Run as a UDP receiver agent (not an attack): listen on this address (e.g. `:9999`), count received packets/bytes, serve them, and exit on Ctrl+C. Run this on your target host. |
| `-receiver-control <addr>` | `:9100` | Receiver mode: HTTP address that serves cumulative received counters as JSON at `/stats`. |
| `-receiver-stats <host:port>` | none | Sender side: poll a receiver agent's stats endpoint to show real landed pps/bps and packet loss in the dashboard (`UDP RECV` row). |

### Tuning

These expose what used to be hardcoded constants. Each also lives in the config file (keys in parentheses); `0` means "use the built in default". (The UDP reflector/amplifier flags live under [UDP reflection / amplifier discovery](#udp-reflection--amplifier-discovery) above.)
...
| flag | default | what it does |
|---|---|---|
| `-bandwidth <rate>` | 2 MB/s | Cap **UDP egress** to a byte/sec budget so a flood paces to your uplink instead of saturating it. Accepts `40mbit`, `5MB`, `500kbit`, `1gbit`, or a plain bytes/sec number (bit rate suffixes are bits; byte suffixes are bytes). Empty defaults to a conservative 2 MB/s; set it to ~your real upload speed. (`bandwidth`) |
...
> `-bandwidth` paces the UDP flood (where uncapped byte blasting is the problem); HTTP attacks are paced by `-rate` plus the adaptive backoff. Both the direct UDP flood **and** the raw-socket reflection path (type 10 under root) draw from the same `-bandwidth` egress budget, and the adaptive backoff throttles either one. Example: `./demon -attack 10 -bandwidth 40mbit -duration 5m <target>` blasts UDP at ~40 Mbit/s flat instead of self-congesting.
...
Sends real gzip compressed request bodies that decompress to 64MB on the server. Zeros compress at roughly 1000:1, so the wire payload is about 64KB but forces the server to allocate a 64MB decompression buffer per request. Sends 25 concurrent bomb requests per worker invocation. Effective against servers that decompress request bodies before routing (common in middleware stacks). Servers without decompression size limits allocate the full buffer.
...
Two modes. The **default is a direct, non-spoofed flood** against your target with real source IPs: two streams run together — a packet-rate stream (60 goroutines firing 1-byte datagrams, batched via `sendmmsg` on Linux so the pps ceiling is the target's receive rate, not our syscall rate) and a byte-rate stream (60 goroutines firing near-MTU 1400-byte datagrams). The packet stream is capped by `-udp-pps` (default 100k pkt/s) and both share the `-bandwidth` egress budget. Direct mode targets the **real service port only** — pass `host:port`; omitting the port defaults to `:80` with a warning.

The **reflection / amplification mode is opt-in** via `-udp-reflection` (raw sockets, spoofed source IP, third-party DNS/NTP/memcached/SSDP amplifiers, plus a fragmented-packet sub-vector). It needs root / `CAP_NET_RAW`; if raw sockets can't open it logs that and falls back to the direct flood. Pair it with `-dns-amplifiers`, `-ntp-amplifiers`, `-memcached-amplifiers`, `-ssdp-amplifiers`, or `-discover-amplifiers`. Note that current internet defences (RFC 8482, NTP monlist patches, memcached firewalling, and BCP 38 egress filtering) make this mode largely ineffective from a single filtered uplink.

Target parsing: any scheme is stripped before the socket is opened, so `1.2.3.4`, `1.2.3.4:53`, and `https://1.2.3.4` all resolve the same way.

**Measuring it.** Because UDP has no acknowledgement, a send only proves the datagram left your box — not that it landed. The dashboard reflects that: it never counts a local write as a verified hit, it splits the signal into a `UDP SEND` row (sent-side rate, plus a `[pps cap binding]` note when `-udp-pps` is the limiter) and a `UDP RECV` row (what a receiver agent absorbed, with real loss), and it uses a UDP-aware health probe instead of an HTTP HEAD (so a healthy UDP service isn't misreported as down). For landed numbers, run the receiver agent on your target (`-udp-receiver :9999 -receiver-control :9100`) and point the sender at it with `-receiver-stats <target>:9100`. Without an agent the dashboard shows sent-side only and says so. The reflector telemetry row (verified reflector count, request/response bytes, local handoff attempts) still appears for the reflection mode.
...
# udp flood (direct/measured by default), pass the real service port, e.g. ip:9999
./demon -attack 10 -concurrency 300 -rate 8000 -infinite <target>:9999
```

### UDP examples (direct + reflection)

Use these as starter recipes:

- direct flood: best when you want a real service-port load test with honest sender/receiver telemetry
- reflection mode: best when you already have a valid amplifier inventory and root / `CAP_NET_RAW`

```bash
# good direct flood starter — real service port, measured by a receiver agent on the target.
# on the target host:
./demon -udp-receiver :9999 -receiver-control :9100
# on the sender (start conservative, then raise -udp-pps if the dashboard says the cap is binding):
./demon -attack 10 -concurrency 150 -rate 4000 -infinite -udp-pps 50000 -bandwidth 50mbit -receiver-stats <target>:9100 <target>:9999

# stronger direct flood — useful when you want to push the real UDP service port harder on a fat uplink.
./demon -attack 10 -concurrency 300 -rate 8000 -infinite -udp-pps 250000 -bandwidth 200mbit -receiver-stats <target>:9100 <target>:9999

# good reflection starter — opt-in, root required, discover live amplifiers first.
sudo ./demon -attack 10 -udp-reflection -concurrency 50 -rate 1000 -discover-amplifiers -discover-timeout 30 -discover-count 25 -discover-save -bandwidth 20mbit -adaptive-bandwidth <target>

# reflection with explicit inventories for DNS/NTP/memcached/SSDP reflectors.
sudo ./demon -attack 10 -udp-reflection -concurrency 100 -rate 2000 -dns-amplifiers dns_resolvers.txt -ntp-amplifiers ntp_monlist.txt -memcached-amplifiers memcached.txt -ssdp-amplifiers ssdp.txt -bandwidth 50mbit <target>
```
...
`-max-udp-floods 16` raises the cap from 8 to 16 concurrent invocations, doubling the goroutine count to ~2,400 across all vectors. Each invocation fires DNS, NTP, memcached, fragmented UDP, and, when a list is supplied — SSDP simultaneously. At this level the bottleneck is amplifier availability and your pipe capacity, not goroutines. NTP monlist at up to 556x amplification means even a modest pipe punches well above its weight. Add `-discover-amplifiers` to build a live amplifier list before the flood starts, or pass `-dns-amplifiers`/`-ntp-amplifiers`/`-memcached-amplifiers`/`-ssdp-amplifiers` with files from a Shodan export. `-udp-reflection` is required to select the amplification mode: type 10 now defaults to the direct, non-spoofed flood, so without that flag this recipe runs a plain direct flood instead.

```bash
ulimit -u 100000
sudo ./demon -attack 10 -udp-reflection -concurrency 500 -rate 20000 \
  -max-udp-floods 16 \
...
# terminal 1 network layer: UDP amplification, all five vectors (-udp-reflection selects it; type 10 defaults to direct)
sudo ./demon -attack 10 -udp-reflection -concurrency 300 -rate 10000 \
  -max-udp-floods 12 \
  -infinite <target>
...
demon_config.json is created automatically on first run with default values. Pass `-config myconfig.json` to use a different file. Command line flags override config file values.
...
> **`-rate` and `-concurrency` interact.** Rate is the ceiling across all workers combined. If you set `-concurrency 500 -rate 500`, each worker averages 1 req/s and spends most of its time blocked on the rate limiter. Useful concurrency is roughly `rate × expected_response_time_in_seconds`. A target responding in 100ms can keep `rate × 0.1` workers actively in flight.

---

## Appendix: botnet path

_This section is purely exploratory... a "what might it actually take to build this out" thought exercise. None of it is implemented, none of it is being built, and the existing `Misc./botnet/` code is a proof of concept skeleton, not a foundation for anything real. The point is to be clear about what the gap between a single machine stress tool and a genuinely distributed attack system actually looks like, because that gap is much larger than it might seem from the outside._
...
**The scraped proxy pool is unreliable by design.** Free public proxy lists are mostly dead, misconfigured, or honeypots. The scraper grabs what it can find and validates as it goes, but don't expect a healthy pool out of it. The intended path for serious use is a paid residential proxy provider with a real API; the scraper exists for cases where you have nothing else and are testing an unprotected target. If your run depends on proxies and you're seeing mostly failures, the proxy list is probably the reason.

**The compression bomb (type 7) has uncertain real world impact.** The implementation sends requests with a gzip compressed body that decompresses to 64MB on paper. Whether the target actually decompresses it depends entirely on the middleware stack. Reverse proxies often drop oversized bodies before they reach application code. Servers with body size limits reject them outright. And in practice this type probably lands much less often than the raw numbers suggest. It's included because it works against a certain class of targets, not because it's universally effective...

**The ReDoS payload in type 9 is just speculative.** The `a`×8000 + `!` string triggers catastrophic backtracking on vulnerable patterns in certain regex engines, but most modern runtimes have ReDoS mitigations (linear time regex, match timeout, etc.) and the pattern may just be rejected quickly. Whether it hits depends on whether the target's validation code has an exploitable regex and whether the runtime has no timeout. There is no way to know that externally, so this payload is currently a shot in the dark.

**The TLS fingerprint mimicry is good but not unbeatable.** `-tls-fingerprint` makes the handshake look like Chrome 133 by JA3/JA4. That defeats fingerprint based blocking at the TLS layer. It does not defeat behavioral analysis, IP reputation scoring, ASN filtering, or HTTP layer bot detection. Cloudflare's bot management layer runs above TLS things like browser challenge pages, JS fingerprinting, and behavioral scoring happen after the TLS connection is established. `-tls-fingerprint` removes one layer of detection but not all of them.

**Rate limit bypass is heuristic and not guaranteed.** The bypass system monitors response codes and headers and adjusts timing accordingly. It works well against simple rate limiters that respond predictably with 429s. Against more sophisticated systems that implement silent throttling, shadow queuing, or asymmetric penalties (letting the first N requests through at full speed before quietly degrading), the detector may not have enough signal to adapt correctly. The adaptive rate control in particular is making guesses about the target's behavior based on limited data.

> **The dashboard metrics are approximate.** The stats counters are updated with atomic operations from hundreds of concurrent goroutines. They are eventually consistent under load. The numbers you see during a run are accurate enough to understand what is happening but should not be treated as precise accounting. The latency histogram is sampled, not exhaustive. For type 10 reflector runs, the dashboard also shows the verified reflector inventory and observed request/response byte totals, which are useful for judging the reflection path, not for claiming a verified target hit on their own.

**HTTP/2 flood (type 2) runs 100 streams over one h2 connection per worker.** The h2 connection is shared and persistent, which is the right design for keeping the stream pressure up. But a single connection also means a single TCP flow, and the server's h2 implementation can terminate the connection, reset streams, or apply backpressure at the framing layer in ways that do not show up as failures in the request counter. What the stats show is streams dispatched, not streams that generated some meaningful server load.

**UDP flood (type 10) defaults to a direct, measured flood; reflection is opt-in.** By default type 10 runs a direct, non-spoofed flood against your target's real service port with real source IPs. The reflection/amplification path (DNS, NTP, memcached, SSDP reflectors, spoofed source IP) is opt-in via `-udp-reflection` and needs root (`CAP_NET_RAW`); without the privileges it logs that and falls back to the direct flood. The dashboard splits the signal into a sent-side rate and — when a receiver agent runs on your target (`-udp-receiver` + `-receiver-stats`) — the actual landed rate and packet loss, instead of pretending a raw send proves target-side impact. Reflector inventory and byte totals are shown separately for reflection runs.

**Single machine ceiling.** Everything in this tool runs from one process on one machine. You can push a lot of traffic with high concurrency and a fast connection, but there is a hard ceiling set by your own uplink, CPU, and the OS's socket and fd limits. A 1Gbps home connection maxes out around 100k small HTTP requests per second under ideal conditions. Anything hardened enough to matter will be provisioned well above that. The tool is genuinely useful for testing lightly defended targets, uncovering misconfigurations, and load testing infrastructure you own. Against a properly operated production system with real DDoS mitigation in front of it, one machine will not be enough regardless of what flags you pass.

**Some attack types do not compose as cleanly as the docs imply.** Tag team mode (`-attack 2,7`) randomly distributes workers across types, which is real and functional, but the types were designed independently and some combinations are awkward. Mixing a connection hold type with a high frequency type means a fraction of your workers are just sitting still holding connections while the rest cycle. That might be the intended effect, but it also means the effective worker count for the cycling types is lower than `-concurrency` suggests.
...
**This is not a finished project.** There are known rough edges: the botnet folder is a toy, the proxy scraper is brittle against source sites changing their layout, and the live path still depends on real-world target-side evidence for absolute confirmation of impact. The dashboard now shows per-type latency breakdown for tag-team runs and a UDP reflector telemetry row for reflector-aware runs, but the numbers are still best treated as operational signal rather than forensic proof.
